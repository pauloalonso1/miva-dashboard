...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  try {
    await db.transaction(async (tx) => {
      // 1. Carrega venda + itens antes de deletar
      const rows = await tx.query.vendas.findMany({
        where: eq(vendas.id, id),
        with: { itens: true },
        limit: 1,
      });
      ...
      if (venda.clienteNome) {
        const [cliente] = await tx.select().from(clientes).where(eq(clientes.nome, venda.clienteNome));
        if (cliente) {
